<template>
  <div class="mineCom">
    <mt-header style="-webkit-transform: translateZ(0)" slot="header" class="primary_bg" title="班课详情" :fixed="true">
	...
	<div style="margin-top: 50px;">
		<router-link :to="{path:'/signed',query:{classes_id:this.classes_id}}" slot="left">
		  <mt-button style="margin-bottom: 10px; width: 150px;" type="primary">进行签到</mt-button>
		</router-link>
		<router-link :to="{path:'/code',query:{classes_id:this.classes_id}}" slot="right">
		  <mt-button style="margin-bottom: 10px; width: 150px; background-color: #FFD700; color: white; float:right " >班级二维码</mt-button>
		</router-link>
		<!-- 后台传该生该班课的经验值  -->
		...
		<div slot="content" class="order_content has-header">
		  <div style="margin-top:10px;background:white" v-for="(item,index) in studentList" :key="index">
...
export default {
	components: {
	  StudentSItem: r => { require.ensure([], () => r(require('./component/StudentSItem.vue'))), "StudentSItem" }
	},
  data() {
    return {
      classes_id: "1234",
