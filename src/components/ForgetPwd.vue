<template>
  <div class="loginContainer">
        ...
        <form @submit="register">
          ...
          <div>
            <section class="register_message">
              ...
              >
                {{
                  forgetForm.computeTime > 0
                    ? `(${forgetForm.computeTime}s)已发送`
                    : "获取验证码"
                }}
              </button>
            </section>
            ...
            <section class="register_verification">
              ...
              <section
                class="switch_button"
                :class="forgetForm.showPwd1 ? 'on' : 'off'"
                @click="forgetForm.showPwd1 = !forgetForm.showPwd1"
              >
                <section
                  class="switch_circle"
                  :class="{ right: forgetForm.showPwd1 }"
                ></section>
                <section class="switch_text">
                  {{ forgetForm.showPwd1 ? "abc" : "...." }}
                </section>
              </section>
            </section>
            <section class="register_verification">
              ...
              <section
                class="switch_button"
                :class="forgetForm.showPwd2 ? 'on' : 'off'"
                @click="forgetForm.showPwd2 = !forgetForm.showPwd2"
              >
                <section
                  class="switch_circle"
                  :class="{ right: forgetForm.showPwd2 }"
                ></section>
                <section class="switch_text">
                  {{ forgetForm.showPwd2 ? "abc" : "...." }}
                </section>
              </section>
            </section>
          </div>
          <button class="register_submit" @click="register_submit">
            确定修改
          </button>
        </form>
      </div>
    ...
    <div class="login">
      已有账号？<router-link to="/login"><a>立即登录</a></router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      //   });
      if (
        this.forgetForm.phone.length == 0 ||
        this.forgetForm.code.length == 0 ||
        this.forgetForm.password1.length == 0 ||
        this.forgetForm.password2.length == 0
      ) {
        uni.hideLoading();
      ...
      if (
        !phonereg.test(this.forgetForm.phone) ||
        this.forgetForm.phone.length != 11
      ) {
        uni.hideLoading();
      ...
      //用户名和密码验证
      if (
        this.forgetForm.password1.length < 8 ||
        this.forgetForm.password1.length > 16
      ) {
        uni.hideLoading();
